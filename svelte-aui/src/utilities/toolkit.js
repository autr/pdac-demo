...
export const CompareXY = (a, b, steps) => {
	const aa = (steps !== undefined) ? RoundedXY(a,steps) : CopyXY(a);
	const bb = (steps !== undefined) ? RoundedXY(b,steps) : CopyXY(b);
	const out = (aa.x === bb.x && aa.y === bb.y);
	return out;
};

export const RoundedXY = ( point, steps ) => {
	return {
...
export const BlendXY = ( a, b, smooth ) => {
...
export const Div2NormXY = ( e, div, rot ) => {
	const bounds = div.getBoundingClientRect();
	let p = {
		x: Scale(e.clientX, bounds.left, bounds.left + bounds.width, 0, 1),
		y: Scale(e.clientY, bounds.top, bounds.top + bounds.height, 0, 1)
	}

	if (rot != 0) p = RotateXY( { x: 0.5, y: 0.5} ,p,rot);
	return p;
};
export const Norm2DivXY = ( point, div, rot ) => {
	const bounds = div.getBoundingClientRect();
	const p = {
